...
├── figures/
├── notebooks/
│   └── final_xgboost_kmeanssmote_demo.ipynb
├── result/
├── src/
│   ├── preprocessing.py
│   ├── train_CatBoost.py
...
Contains a clean demonstration notebook for local checking and final model reproduction:

```text
notebooks/final_xgboost_kmeanssmote_demo.ipynb
```

This notebook uses local CSV paths directly:
...
Contains validation outputs, probability files, and intermediate experiment summaries.

### `src/`

Contains Python source code for preprocessing and model training.

The main reproducible training scripts are:
...
### Option 1: Run the final Python script

```bash
python src/train_XGBoost_KMeansSMOTE.py
```

This script trains the XGBoost + KMeansSMOTE route and generates submission files in:

```text
submissions/
```

### Option 2: Run the local demo notebook
...
The notebook assumes the following files are in the project root directory:
...
```text
submissions/final_xgboost_kmeanssmote_submission.csv
submissions/final_xgboost_kmeanssmote_probability.csv
```
