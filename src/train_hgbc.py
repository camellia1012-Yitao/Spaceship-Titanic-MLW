...
class FeatureBuilder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        df["SpendStd"] = spends.std(axis=1)
        df["SpendNonZeroCount"] = (spends > 0).sum(axis=1)
        ...
        df["ServiceShare"] = df["RoomService"].fillna(0) / (df["TotalSpend"] + 1.0)
        ...
        df["CabinRegionFine"] = pd.cut(
            df["CabinNum"],
            bins=[-1, 150, 300, 500, 700, 900, 1200, 1600, 2000],
            labels=["R1", "R2", "R3", "R4", "R5", "R6", "R7", "R8"],
        )
        df["CryoNoSpendMatch"] = ((df["CryoSleep"] == True) & (df["TotalSpend"] == 0)).astype(int)
        df["VIPHighSpendMatch"] = ((df["VIP"] == True) & (df["TotalSpend"] > 0)).astype(int)

        missing_flags = X.isna().astype(int)
        missing_flags.columns = [f"{column}_missing" for column in missing_flags.columns]
        df = pd.concat([df, missing_flags], axis=1)
        df["MissingCount"] = X.isna().sum(axis=1)
...
def main():
    parser = argparse.ArgumentParser(description="Spaceship Titanic solution with engineered features and an HGBC ensemble.")
    parser.add_argument(
        "--train",
    ...
    parser.add_argument(
        "--output",
        default="submissions/hgbc_3model_submission.csv",
    ...
    print("Models: weighted ensemble of HistGradientBoostingClassifier pipelines")
